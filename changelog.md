...
- Test coverage for: fact/state reuse across repeated `run()` calls, the `overwrite=false` behavior of `givenAll()`, a `then()` consumer throwing mid-chain, and running a detached `Rule`

### Fixed

- `RuleBook`/`Rule` no longer leak facts across repeated `run()` calls on the same instance. Facts passed to `run()` are now merged for that invocation only and are never written back into the sticky fact store set via `given()`/`givenAll()`
